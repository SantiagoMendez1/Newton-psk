    ...
    def model_hyperparam_tuning(self, run=False):
        ...
        if run is False:
            try:
                ...
                self.result_query_best_params = db.execute_query(self.database, 
                                                                 self.query)
        ...
        if run:
            ...
            for products in products_to_train:
                X_train = self.train[['week', 'year']]
                y_train = self.train[products]
                X_test = self.test[['week', 'year']]
                y_test = self.test[products]
            ...
            df_product_trained.to_sql(name="product_trained", 
                                      con=self.engine,                  
                                      if_exists='append', 
                                      index=False)
            return None
